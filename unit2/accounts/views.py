...
class SocialLoginView(View):
    ...
    def get(self, request):
        exists = User.exists(request.GET.get('email'))
    ...
    def post(self, request):
        ...
        created = False
        if not user:
            user, created = User.create_user(email)
            if request.POST.get('usedAPI') == self.facebookAPI:
                user.facebook_id = request.POST.get('id')
            else:
                user.google_id = request.POST.get('id')
            user.first_name = request.POST.get('first_name')
            user.last_name = request.POST.get('last_name')
            user.username = request.POST.get('name')
            user.nickname = request.POST.get('name')
            user.avatar = request.POST.get('avatar_url')
            user.is_active = True
            user.emailed = True
            user.save()

        if user:
            if request.POST.get('usedAPI') == self.facebookAPI and not user.facebook_id:
                user.facebook_id = request.POST.get('id')

            if request.POST.get('usedAPI') == self.googleAPI and not user.google_id:
                user.google_id = request.POST.get('id')
        ...
        cache.clear()
        login(request, user)
        return JsonResponse({'authenticated': True, 'new_csrf_token': request.COOKIES['csrftoken'], 'is_new': created})
